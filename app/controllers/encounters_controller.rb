class EncountersController < ApplicationController
  ...
  def player_level_avg(encounter)
    ...
    average_player_level = (sum / char_objs.count)
    average_player_level
  end

  def determine_challenge_rating(encounter)
    difference = ( monster_challenge_sum(encounter) - player_level_avg(encounter) )
  ...
  def very_easy_monsters
    @very_easy_monsters = Monster.where(challenge_rating: 0...1)
  end

  def easy_monsters
    @easy_monsters = Monster.where(challenge_rating: [1..3])
  end

  def medium_monsters
    @medium_monsters = Monster.where(challenge_rating: [4..7])
  end

  def hard_monsters
    @hard_monsters = Monster.where(challenge_rating: [8..10])
  end

  def very_hard_monsters
    @very_hard_monsters = Monster.where(challenge_rating: [11..20])
  end

  def impossible_monsters
    @impossible_monsters = Monster.where(challenge_rating: [21..30])
  end
